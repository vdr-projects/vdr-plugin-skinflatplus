...
void cFlatBaseRender::CreateFullOsd(void) {
  CreateOsd(cOsd::OsdLeft() + Config.marginOsdHor,
            cOsd::OsdTop() + Config.marginOsdVer,
            cOsd::OsdWidth() - Config.marginOsdHor * 2,
            cOsd::OsdHeight() - Config.marginOsdVer * 2);
}
...
void cFlatBaseRender::CreateOsd(int left, int top, int width, int height) {
  ...
  esyslog(
      "skinflatplus: create osd FAILED left: %d top: %d width: %d height: %d",
      left, top, width, height);
  return;
}

cPixmap *cFlatBaseRender::CreatePixmap(int Layer, const cRect &ViewPort,
...
void cFlatBaseRender::TopBarCreate(void) {
  ...
  topBarFontClock = cFont::CreateFont(Setup.FontOsd,
                                      fs * Config.TopBarFontClockScale * 100.0);
  ...
  topBarPixmap = CreatePixmap(
      1, cRect(Config.decorBorderTopBarSize, Config.decorBorderTopBarSize,
  ...
  topBarIconBGPixmap = CreatePixmap(
      2, cRect(Config.decorBorderTopBarSize, Config.decorBorderTopBarSize,
               osdWidth - Config.decorBorderTopBarSize * 2, topBarHeight));
  ...
  // osdWidth - Config.decorBorderTopBarSize*2, topBarHeight);
  topBarIconPixmap = CreatePixmap(
      3, cRect(Config.decorBorderTopBarSize, Config.decorBorderTopBarSize,
               osdWidth - Config.decorBorderTopBarSize * 2, topBarHeight));
...
void cFlatBaseRender::TopBarEnableDiskUsage(void) {
  ...
  if (Config.DiskUsageFree == 1) {        // Show in free mode
    ...
    if (Config.DiskUsageShort == false) { // Long format
      extra1 = cString::sprintf("%s: %d%% %s", tr("Disk"), ChartDiskUsage,
                                tr("free"));
      if (FreeGB < 1000.0) { // Less than 1000 GB
        extra2 = cString::sprintf("%.1f GB ~ %02d:%02d", FreeGB,
                                  FreeMinutes / 60, FreeMinutes % 60);
      } else { // 1000 GB+
        extra2 = cString::sprintf("%.2f TB ~ %02d:%02d", FreeGB / 1024.0,
                                  FreeMinutes / 60, FreeMinutes % 60);
      }
    } else { // Short format
      extra1 = cString::sprintf("%d%% %s", ChartDiskUsage, tr("free"));
      extra2 =
          cString::sprintf("~ %02d:%02d", FreeMinutes / 60, FreeMinutes % 60);
    }
    ...
    if (Config.DiskUsageShort == false) { // Long format
      extra1 = cString::sprintf("%s: %d%% %s", tr("Disk"), DiskUsage,
                                tr("occupied"));
      if (OccupiedGB < 1000.0) { // Less than 1000 GB
        extra2 = cString::sprintf("%.1f GB ~ %02d:%02d", OccupiedGB,
                                  OccupiedMinutes / 60, OccupiedMinutes % 60);
      } else { // 1000 GB+
        extra2 = cString::sprintf("%.2f TB ~ %02d:%02d", OccupiedGB / 1024.0,
                                  OccupiedMinutes / 60, OccupiedMinutes % 60);
      }
    } else { // Short format
      extra1 = cString::sprintf("%d%% %s", DiskUsage, tr("occupied"));
      extra2 = cString::sprintf("~ %02d:%02d", OccupiedMinutes / 60,
                                OccupiedMinutes % 60);
    }
...
void cFlatBaseRender::TopBarUpdate(void) {
  ...
  if (strcmp(curDate, topBarLastDate) || topBarUpdateTitle) {
    ...
    if (topBarMenuIconSet && Config.TopBarMenuIconShow) {
      ...
      cImage *img = imgLoader.LoadIcon(*topBarMenuIcon, 999,
                                       topBarHeight - marginItem * 2);
      if (img) {
    ...
    if (topBarMenuLogoSet && Config.TopBarMenuIconShow) {
      ...
      cImage *imgBG =
          imgLoader.LoadIcon("logo_background", imageBGWidth, imageBGHeight);
      if (imgBG) {
      ...
      cImage *img = imgLoader.LoadLogo(*topBarMenuLogo, imageBGWidth - 4,
                                       imageBGHeight - 4);
      if (img) {
    ...
    topBarPixmap->DrawText(cPoint(Right, fontClockTop), time2,
                           Theme.Color(clrTopBarTimeFont),
    ...
    Right = TopBarWidth - timeWidth - std::max(weekdayWidth, dateWidth) -
            marginItem;
    topBarPixmap->DrawText(cPoint(Right, fontSmlTop), weekday,
                           Theme.Color(clrTopBarDateFont),
                           Theme.Color(clrTopBarBg), topBarFontSml,
                           std::max(weekdayWidth, dateWidth), 0, taRight);
    topBarPixmap->DrawText(cPoint(Right, fontSmlTop + topBarFontSmlHeight),
                           date, Theme.Color(clrTopBarDateFont),
                           Theme.Color(clrTopBarBg), topBarFontSml,
                           std::max(weekdayWidth, dateWidth), 0, taRight);
    ...
    if (Config.TopBarRecConflictsShow) {
      ...
      if (p) {
        Epgsearch_lastconflictinfo_v1_0 *serviceData =
            new Epgsearch_lastconflictinfo_v1_0;
        if (serviceData) {
      ...
      if (numConflicts) {
        if (numConflicts < Config.TopBarRecConflictsHigh)
          imgCon = imgLoader.LoadIcon("topbar_timerconflict_low",
                                      topBarFontHeight - marginItem * 2,
                                      topBarFontHeight - marginItem * 2);
        else
          imgCon = imgLoader.LoadIcon("topbar_timerconflict_high",
                                      topBarFontHeight - marginItem * 2,
                                      topBarFontHeight - marginItem * 2);
        ...
        if (imgCon) {
          ...
          middleWidth +=
              imgCon->Width() + topBarFontSml->Width(*Con) + marginItem;
        }
      }
    }
...
// look for timers
...
#endif
      if (numRec) {
        imgRec = imgLoader.LoadIcon("topbar_timer",
                                    topBarFontHeight - marginItem * 2,
                                    topBarFontHeight - marginItem * 2);
        if (imgRec) {
          ...
          middleWidth +=
              imgRec->Width() + topBarFontSml->Width(*Rec) + marginItem;
        }
      }
    }
    ...
    if (topBarFont->Width(topBarTitle) + topBarMenuIconRightWidth >
        titleMaxWidth) {
      topBarMenuIconRightLeft = titleMaxWidth + marginItem * 2;
      titleMaxWidth -= topBarMenuIconRightWidth;
    } else {
      topBarMenuIconRightLeft =
          titleLeft + topBarFont->Width(topBarTitle) + marginItem * 2;
    }

    topBarPixmap->DrawText(cPoint(Right, fontSmlTop), tobBarTitleExtra1,
                           Theme.Color(clrTopBarDateFont),
                           Theme.Color(clrTopBarBg), topBarFontSml,
                           extraMaxWidth, 0, taRight);
    topBarPixmap->DrawText(cPoint(Right, fontSmlTop + topBarFontSmlHeight),
                           tobBarTitleExtra2, Theme.Color(clrTopBarDateFont),
                           Theme.Color(clrTopBarBg), topBarFontSml,
                           extraMaxWidth, 0, taRight);
    Right += extraMaxWidth + marginItem;
    ...
    if (numRec && imgRec) {
      ...
      topBarPixmap->DrawText(cPoint(Right, fontSmlTop), RecNum,
                             Theme.Color(clrTopBarRecordingActiveFg),
                             Theme.Color(clrTopBarRecordingActiveBg),
                             topBarFontSml);
    ...
    if (numConflicts && imgCon) {
      ...
      if (numConflicts < Config.TopBarRecConflictsHigh)
        topBarPixmap->DrawText(cPoint(Right, fontSmlTop), ConNum,
                               Theme.Color(clrTopBarConflictLowFg),
                               Theme.Color(clrTopBarConflictLowBg),
                               topBarFontSml);
      else
        topBarPixmap->DrawText(cPoint(Right, fontSmlTop), ConNum,
                               Theme.Color(clrTopBarConflictHighFg),
                               Theme.Color(clrTopBarConflictHighBg),
                               topBarFontSml);
    ...
    if (topBarMenuIconRightSet) {
      ...
      if (img) {
        int iconTop = (topBarHeight / 2 - img->Height() / 2);
        topBarIconPixmap->DrawImage(cPoint(topBarMenuIconRightLeft, iconTop),
                                    *img);
      }
    }
    topBarPixmap->DrawText(cPoint(titleLeft, fontTop), topBarTitle,
                           Theme.Color(clrTopBarFont), Theme.Color(clrTopBarBg),
                           topBarFont, titleMaxWidth);

    DecorBorderDraw(Config.decorBorderTopBarSize, Config.decorBorderTopBarSize,
                    osdWidth - Config.decorBorderTopBarSize * 2, topBarHeight,
                    Config.decorBorderTopBarSize, Config.decorBorderTopBarType,
                    Config.decorBorderTopBarFg, Config.decorBorderTopBarBg);
  }
}
...
void cFlatBaseRender::ButtonsCreate(void) {
  ...
  buttonsPixmap = CreatePixmap(
      1, cRect(Config.decorBorderButtonSize, buttonsTop,
...
void cFlatBaseRender::ButtonsSet(const char *Red, const char *Green,
  ...
  if (!(!Config.ButtonsShowEmpty && !Red)) {
    ...
    case 0:
      buttonsPixmap->DrawText(cPoint(x, 0), Red, Theme.Color(clrButtonFont),
                              Theme.Color(clrButtonBg), font, buttonWidth,
                              fontHeight + marginButtonColor, taCenter);
      buttonsPixmap->DrawRectangle(cRect(x, fontHeight + marginButtonColor,
                                         buttonWidth, buttonColorHeight),
                                   Theme.Color(clrButtonRed));
      break;
    case 1:
      buttonsPixmap->DrawText(cPoint(x, 0), Green, Theme.Color(clrButtonFont),
                              Theme.Color(clrButtonBg), font, buttonWidth,
                              fontHeight + marginButtonColor, taCenter);
      buttonsPixmap->DrawRectangle(cRect(x, fontHeight + marginButtonColor,
                                         buttonWidth, buttonColorHeight),
      ...
      break;
    case 2:
      buttonsPixmap->DrawText(cPoint(x, 0), Yellow, Theme.Color(clrButtonFont),
                              Theme.Color(clrButtonBg), font, buttonWidth,
                              fontHeight + marginButtonColor, taCenter);
      buttonsPixmap->DrawRectangle(cRect(x, fontHeight + marginButtonColor,
                                         buttonWidth, buttonColorHeight),
      ...
      break;
    case 3:
      buttonsPixmap->DrawText(cPoint(x, 0), Blue, Theme.Color(clrButtonFont),
                              Theme.Color(clrButtonBg), font, buttonWidth,
                              fontHeight + marginButtonColor, taCenter);
      buttonsPixmap->DrawRectangle(cRect(x, fontHeight + marginButtonColor,
                                         buttonWidth, buttonColorHeight),
    ...
    DecorBorderDraw(x + Config.decorBorderButtonSize, buttonsTop, buttonWidth,
                    buttonsHeight, Config.decorBorderButtonSize,
                    Config.decorBorderButtonType, Config.decorBorderButtonFg,
  ...
  if (x + buttonWidth + Config.decorBorderButtonSize * 2 < buttonsWidth)
    buttonWidth +=
        buttonsWidth - (x + buttonWidth + Config.decorBorderButtonSize * 2);
...
void cFlatBaseRender::MessageCreate(void) {
  ...
  if (Config.MessageColorPosition == 1)
    messageHeight += 8;
  int top = osdHeight - Config.MessageOffset - messageHeight -
            Config.decorBorderMessageSize;
  messagePixmap = CreatePixmap(
      5, cRect(Config.decorBorderMessageSize, top,
               osdWidth - Config.decorBorderMessageSize * 2, messageHeight));
...
void cFlatBaseRender::MessageSet(eMessageType Type, const char *Text) {
  ...
  if (Config.MessageColorPosition == 0) {
    ...
    messagePixmap->DrawRectangle(
        cRect(osdWidth - 8 - Config.decorBorderMessageSize * 2, 0, 8,
              messageHeight),
        col);
  } else {
  ...
  int maxWidth = osdWidth - Config.decorBorderMessageSize * 2 - fontHeight -
                 marginItem * 3 - 10;
  ...
  if ((textWidth > maxWidth) && Config.ScrollerEnable) {
    messageScroller.AddScroller(
        ...
        cRect(Config.decorBorderMessageSize + fontHeight + marginItem * 3 + 10,
              osdHeight - Config.MessageOffset - messageHeight -
                  Config.decorBorderMessageSize,
              maxWidth, fontHeight),
    ...
    if (found != std::string::npos) {
      ...
      messagePixmap->DrawText(cPoint((osdWidth - textWidth) / 2, marginItem),
                              first.c_str(), Theme.Color(clrMessageFont),
                              Theme.Color(clrMessageBg), font);
      int l = font->Width(first.c_str());
      messagePixmap->DrawText(
          cPoint((osdWidth - textWidth) / 2 + l, marginItem), second.c_str(),
          Theme.Color(clrMenuItemExtraTextFont), Theme.Color(clrMessageBg),
          font);
      ...
      if ((textWidth > maxWidth) && Config.ScrollerEnable)
        messageScroller.AddScroller(
            Text,
            cRect(Config.decorBorderMessageSize + fontHeight + marginItem * 3 +
                      10,
                  osdHeight - Config.MessageOffset - messageHeight -
                      Config.decorBorderMessageSize,
                  maxWidth, fontHeight),
        ...
        messagePixmap->DrawText(cPoint((osdWidth - textWidth) / 2, marginItem),
                                Text, Theme.Color(clrMessageFont),
                                Theme.Color(clrMessageBg), font);
    }
  } else {
    if ((textWidth > maxWidth) && Config.ScrollerEnable)
  ...
  int top = osdHeight - Config.MessageOffset - messageHeight -
            Config.decorBorderMessageSize;
  DecorBorderDraw(Config.decorBorderMessageSize, top,
                  osdWidth - Config.decorBorderMessageSize * 2, messageHeight,
                  Config.decorBorderMessageSize, Config.decorBorderMessageType,
                  Config.decorBorderMessageFg, Config.decorBorderMessageBg,
                  BorderMessage);
}
...
void cFlatBaseRender::ProgressBarCreate(int Left, int Top, int Width,
                                        int Height, int MarginHor,
                                        int MarginVer, tColor ColorFg,
                                        tColor ColorBarFg, tColor ColorBg,
                                        int Type, bool SetBackground,
                                        bool isSignal) {
  progressBarTop = Top;
  ...
  progressBarPixmap =
      CreatePixmap(3, cRect(Left, Top, Width, progressBarHeight));
  progressBarPixmapBg = CreatePixmap(
      2, cRect(Left - progressBarMarginVer, Top - progressBarMarginHor,
...
void cFlatBaseRender::ProgressBarDraw(int Current, int Total) {
  ProgressBarDrawRaw(progressBarPixmap, progressBarPixmapBg,
                     cRect(0, 0, progressBarWidth, progressBarHeight),
                     cRect(0, 0, progressBarWidth + progressBarMarginVer * 2,
                           progressBarHeight + progressBarMarginHor * 2),
                     Current, Total, progressBarColorFg, progressBarColorBarFg,
                     progressBarColorBg, ProgressType, progressBarSetBackground,
                     progressBarIsSignal);
}
...
void cFlatBaseRender::ProgressBarDrawRaw(cPixmap *Pixmap, cPixmap *PixmapBg,
                                         cRect rect, cRect rectBg, int Current,
                                         int Total, tColor ColorFg,
                                         tColor ColorBarFg, tColor ColorBg,
                                         int Type, bool SetBackground,
                                         bool isSignal) {
  ...
  if (PixmapBg && SetBackground)
    PixmapBg->DrawRectangle(
        cRect(rectBg.Left(), rectBg.Top(), rectBg.Width(), rectBg.Height()),
        ColorBg);

  if (SetBackground) {
    if (PixmapBg == Pixmap)
      Pixmap->DrawRectangle(
          cRect(rect.Left(), rect.Top(), rect.Width(), rect.Height()), ColorBg);
    else
      Pixmap->DrawRectangle(
          cRect(rect.Left(), rect.Top(), rect.Width(), rect.Height()),
          clrTransparent);
  }
  switch (Type) {
  case 0: // small line + big line
  {
    ...
    Pixmap->DrawRectangle(
        cRect(rect.Left(), rect.Top() + Middle - (sml / 2), rect.Width(), sml),
        ColorFg);

    if (Current > 0)
      Pixmap->DrawRectangle(cRect(rect.Left(), rect.Top() + Middle - (big / 2),
                                  (rect.Width() * percentLeft), big),
                            ColorBarFg);
    break;
  }
  case 1: // big line
  ...
  case 2: // big line + outline
  {
    ...
    if (rect.Height() > 10)
      out = 2;
    // outline
    Pixmap->DrawRectangle(cRect(rect.Left(), rect.Top(), rect.Width(), out),
                          ColorFg);
    Pixmap->DrawRectangle(
        cRect(rect.Left(), rect.Top() + rect.Height() - out, rect.Width(), out),
        ColorFg);

    Pixmap->DrawRectangle(cRect(rect.Left(), rect.Top(), out, rect.Height()),
                          ColorFg);
    Pixmap->DrawRectangle(
        cRect(rect.Left() + rect.Width() - out, rect.Top(), out, rect.Height()),
        ColorFg);
    ...
    if (Current > 0) {
      if (isSignal) {
        ...
        if (perc > 0.666) {
          Pixmap->DrawRectangle(
              cRect(rect.Left() + out, rect.Top() + Middle - (big / 2) + out,
  ...
  case 3: // small line + big line + dot
  {
    ...
    if (Current > 0) {
      ...
      // dot
      Pixmap->DrawEllipse(
          cRect(rect.Left() + (rect.Width() * percentLeft) - (big / 2),
  ...
  case 8: // small line + big line + alpha blend
  {
    ...
    if (Current > 0) {
      DecorDrawGlowRectHor(Pixmap, rect.Left(), rect.Top(),
                           (rect.Width() * percentLeft), big, ColorBarFg);
      DecorDrawGlowRectHor(Pixmap, rect.Left(), rect.Top() + Middle + sml / 2,
                           (rect.Width() * percentLeft), big * -1, ColorBarFg);
    }
    break;
  }
  case 9: // big line + alpha blend
  {
    ...
    if (Current > 0) {
      DecorDrawGlowRectHor(Pixmap, rect.Left(), rect.Top() + Middle - big / 2,
                           (rect.Width() * percentLeft), big / 2, ColorBarFg);
      DecorDrawGlowRectHor(Pixmap, rect.Left(), rect.Top() + Middle,
                           (rect.Width() * percentLeft), big / -2, ColorBarFg);
    }
    break;
  }
...
void cFlatBaseRender::ProgressBarDrawMarks(int Current, int Total,
                                           const cMarks *Marks, tColor Color,
                                           tColor ColorCurrent) {
  ...
  if (progressBarPixmapBg)
    progressBarPixmapBg->DrawRectangle(
        cRect(0, progressBarMarginHor + progressBarHeight, progressBarWidth,
              progressBarMarginHor),
        progressBarColorBg);
  ...
  progressBarPixmap->DrawRectangle(
      cRect(0, top - sml / 2, progressBarWidth, sml), progressBarColorFg);
  ...
  for (const cMark *m = Marks->First(); m; m = Marks->Next(m)) {
    ...
    ProgressBarDrawMark(posMark, posMarkLast, posCurrent, Start,
                        m->Position() == Current);
    posMarkLast = posMark;
    Start = !Start;
  }
  ...
  if (posCurrent == posMark)
    progressBarPixmap->DrawRectangle(
        cRect(posMark - sml, 0, sml * 2, progressBarHeight),
        progressBarColorMarkCurrent);
  else
    progressBarPixmap->DrawRectangle(
        cRect(posMark - sml / 2, 0, sml, progressBarHeight),
  ...
  if (!Start) {
    ...
    if (posCurrent > posMarkLast)
      progressBarPixmap->DrawRectangle(cRect(posMarkLast + sml / 2,
                                             top - big / 2,
                                             posCurrent - posMarkLast, big),
                                       progressBarColorBarCurFg);
  } else {
    // marker
    progressBarPixmap->DrawRectangle(
        cRect(posMarkLast, top - sml / 2, posCurrent - posMarkLast, sml),
        progressBarColorBarCurFg);
    progressBarPixmap->DrawRectangle(
        cRect(posCurrent - big / 2, top - big / 2, big, big),
        progressBarColorBarCurFg);

    if (posCurrent > posMarkLast + sml / 2)
      progressBarPixmap->DrawRectangle(
          cRect(posMarkLast - sml / 2, 0, sml, progressBarHeight),
          progressBarColorMark);
  }
}
...
void cFlatBaseRender::ProgressBarDrawMark(int posMark, int posMarkLast,
                                          int posCurrent, bool Start,
                                          bool isCurrent) {
  int top = progressBarHeight / 2;
  int sml = Config.decorProgressReplaySize / 10 * 2;
  if (sml <= 4)
  ...
  if (Start) {
    if (posCurrent > posMark)
      progressBarPixmap->DrawRectangle(
          cRect(posMarkLast, top - sml / 2, posMark - posMarkLast, sml),
          progressBarColorBarCurFg);
    else {
      // marker
      progressBarPixmap->DrawRectangle(
    ...
    // marker top
    if (isCurrent)
      progressBarPixmap->DrawRectangle(cRect(posMark - mbig / 2, 0, mbig, sml),
                                       progressBarColorMarkCurrent);
    else
      progressBarPixmap->DrawRectangle(cRect(posMark - mbig / 2, 0, mbig, sml),
                                       progressBarColorMark);
    ...
    if (posCurrent > posMark) {
      progressBarPixmap->DrawRectangle(
          cRect(posMarkLast, top - big / 2, posMark - posMarkLast, big),
          progressBarColorBarCurFg);
      // draw last marker top
      progressBarPixmap->DrawRectangle(
          cRect(posMarkLast - mbig / 2, 0, mbig, marginItem / 2),
          progressBarColorMark);
    } else {
      progressBarPixmap->DrawRectangle(
          cRect(posMarkLast, top - big / 2, posMark - posMarkLast, big),
      ...
      if (posCurrent > posMarkLast) {
        progressBarPixmap->DrawRectangle(
            cRect(posMarkLast, top - big / 2, posCurrent - posMarkLast, big),
            progressBarColorBarCurFg);
        // draw last marker top
        progressBarPixmap->DrawRectangle(
            cRect(posMarkLast - mbig / 2, 0, mbig, marginItem / 2),
    ...
    if (isCurrent)
      progressBarPixmap->DrawRectangle(
          cRect(posMark - mbig / 2, progressBarHeight - sml, mbig, sml),
          progressBarColorMarkCurrent);
    else
      progressBarPixmap->DrawRectangle(
          cRect(posMark - mbig / 2, progressBarHeight - sml, mbig, sml),
  ...
  if (posCurrent == posMarkLast && posMarkLast != 0)
    progressBarPixmap->DrawRectangle(
        cRect(posMarkLast - sml, 0, sml * 2, progressBarHeight),
        progressBarColorMarkCurrent);
...
void cFlatBaseRender::ScrollbarDraw(cPixmap *Pixmap, int Left, int Top,
                                    int Height, int Total, int Offset,
                                    int Shown, bool CanScrollUp,
                                    bool CanScrollDown) {
  if (!Pixmap)
    return;

  int scrollHeight = std::max(int((Height) * double(Shown) / Total + 0.5), 5);
  int scrollTop = std::min(int(Top + (Height) * double(Offset) / Total + 0.5),
  ...
  if (Total > 0 && Total > Shown) {
    ...
    Pixmap->DrawRectangle(cRect(Left, Top, scrollBarWidth, Height),
                          Config.decorScrollBarBg);
    switch (Type) {
    default:
    case 0: {
      int lineWidth = 6;
      if (scrollBarWidth <= 10)
        ...
        lineWidth = 4;
      Pixmap->DrawRectangle(cRect(Left, Top, lineWidth, Height),
                            Config.decorScrollBarFg);

      // Bar
      Pixmap->DrawRectangle(cRect(Left + lineWidth, scrollTop,
                                  scrollBarWidth - lineWidth, scrollHeight),
    ...
    case 1: {
      ...
      // Bar
      Pixmap->DrawRectangle(cRect(Left + lineWidth, scrollTop + dotHeight,
                                  scrollBarWidth - lineWidth,
                                  scrollHeight - dotHeight * 2),
                            Config.decorScrollBarBarFg);
      // dot
      Pixmap->DrawEllipse(cRect(Left + lineWidth, scrollTop,
                                scrollBarWidth - lineWidth,
                                scrollBarWidth - lineWidth),
                          Config.decorScrollBarBarFg, 0);
      Pixmap->DrawEllipse(
          cRect(Left + lineWidth, scrollTop + scrollHeight - dotHeight * 2,
    ...
    case 2: {
      ...
      Pixmap->DrawRectangle(
          cRect(Middle - lineWidth / 2, Top, lineWidth, Height),
          Config.decorScrollBarFg);
      // Bar
      Pixmap->DrawRectangle(
          cRect(Left, scrollTop, scrollBarWidth, scrollHeight),
          Config.decorScrollBarBarFg);
      break;
    }
    case 3: {
      ...
      // Bar
      Pixmap->DrawRectangle(cRect(Left, scrollTop + dotHeight, scrollBarWidth,
                                  scrollHeight - dotHeight * 2),
                            Config.decorScrollBarBarFg);
      // dot
      Pixmap->DrawEllipse(
          cRect(Left, scrollTop, scrollBarWidth, scrollBarWidth),
          Config.decorScrollBarBarFg, 0);
      Pixmap->DrawEllipse(cRect(Left, scrollTop + scrollHeight - dotHeight * 2,
    ...
    case 4: {
      ...
      if (scrollBarWidth > 10)
        out = 2;
      // outline
      Pixmap->DrawRectangle(cRect(Left, Top, scrollBarWidth, out),
                            Config.decorScrollBarFg);
      Pixmap->DrawRectangle(
          cRect(Left, Top + Height - out, scrollBarWidth, out),
          Config.decorScrollBarFg);
      Pixmap->DrawRectangle(cRect(Left, Top, out, Height),
                            Config.decorScrollBarFg);
      Pixmap->DrawRectangle(
          cRect(Left + scrollBarWidth - out, Top, out, Height),
          Config.decorScrollBarFg);

      // bar
      Pixmap->DrawRectangle(cRect(Left + out, scrollTop + out,
    ...
    case 5: {
      ...
      // bar
      Pixmap->DrawRectangle(cRect(Left + out, scrollTop + dotHeight + out,
                                  scrollBarWidth - out * 2,
                                  scrollHeight - dotHeight * 2 - out * 2),
                            Config.decorScrollBarBarFg);
      // dot
      Pixmap->DrawEllipse(cRect(Left + out, scrollTop + out,
                                scrollBarWidth - out * 2,
                                scrollBarWidth - out * 2),
                          Config.decorScrollBarBarFg, 0);
      Pixmap->DrawEllipse(
          cRect(Left + out, scrollTop + scrollHeight - dotHeight * 2 + out,
...
void cFlatBaseRender::DecorBorderClear(int Left, int Top, int Width, int Height,
                                       int Size) {
  ...
  if (decorPixmap) {
    // top
    decorPixmap->DrawRectangle(cRect(LeftDecor, TopDecor, WidthDecor, Size),
                               clrTransparent);
    // right
    decorPixmap->DrawRectangle(
        cRect(LeftDecor + Size + Width, TopDecor, Size, HeightDecor),
        clrTransparent);
    // bottom
    decorPixmap->DrawRectangle(
        cRect(LeftDecor, TopDecor + BottomDecor, WidthDecor, Size),
        clrTransparent);
    // left
    decorPixmap->DrawRectangle(cRect(LeftDecor, TopDecor, Size, HeightDecor),
                               clrTransparent);
  }
}
...
void cFlatBaseRender::DecorBorderClearByFrom(int From) {
  ...
  for (it = Borders.begin(); it != Borders.end();) {
    if ((*it).From == From) {
      DecorBorderClear((*it).Left, (*it).Top, (*it).Width, (*it).Height,
                       (*it).Size);
      it = Borders.erase(it);
    } else
      ++it;
...
void cFlatBaseRender::DecorBorderRedrawAll(void) {
  ...
  for (it = Borders.begin(); it != Borders.end(); it++) {
    DecorBorderDraw((*it).Left, (*it).Top, (*it).Width, (*it).Height,
                    (*it).Size, (*it).Type, (*it).ColorFg, (*it).ColorBg,
                    (*it).From, false);
  }
}
...
void cFlatBaseRender::DecorBorderDraw(int Left, int Top, int Width, int Height,
                                      int Size, int Type, tColor ColorFg,
  ...
  if (!decorPixmap) {
    decorPixmap =
        CreatePixmap(4, cRect(0, 0, cOsd::OsdWidth(), cOsd::OsdHeight()));
    decorPixmap->Fill(clrTransparent);
  }
  ...
  case 1: // rect
    ...
    // right
    decorPixmap->DrawRectangle(
        cRect(LeftDecor + Size + Width, TopDecor, Size, HeightDecor), ColorBg);
    // bottom
    decorPixmap->DrawRectangle(
        cRect(LeftDecor, TopDecor + BottomDecor, WidthDecor, Size), ColorBg);
    // left
    decorPixmap->DrawRectangle(cRect(LeftDecor, TopDecor, Size, HeightDecor),
                               ColorBg);
    break;
  case 2: // round
    // top
    decorPixmap->DrawRectangle(cRect(LeftDecor + Size, TopDecor, Width, Size),
                               ColorBg);
    // right
    decorPixmap->DrawRectangle(
        cRect(LeftDecor + Size + Width, TopDecor + Size, Size, Height),
        ColorBg);
    // bottom
    decorPixmap->DrawRectangle(
        cRect(LeftDecor + Size, TopDecor + BottomDecor, Width, Size), ColorBg);
    // left
    decorPixmap->DrawRectangle(cRect(LeftDecor, TopDecor + Size, Size, Height),
                               ColorBg);

    // top,left corner
    decorPixmap->DrawEllipse(cRect(LeftDecor, TopDecor, Size, Size), ColorBg,
                             2);
    // top,right corner
    decorPixmap->DrawEllipse(
        cRect(LeftDecor + Size + Width, TopDecor, Size, Size), ColorBg, 1);
    // bottom,left corner
    decorPixmap->DrawEllipse(
        cRect(LeftDecor, TopDecor + BottomDecor, Size, Size), ColorBg, 3);
    // bottom,right corner
    decorPixmap->DrawEllipse(
        cRect(LeftDecor + Size + Width, TopDecor + BottomDecor, Size, Size),
        ColorBg, 4);
    break;
  case 3: // invert round
    // top
    ...
    // top,right corner
    decorPixmap->DrawEllipse(
        cRect(LeftDecor + Size + Width, TopDecor, Size, Size), ColorBg, -3);
    // bottom,left corner
    decorPixmap->DrawEllipse(
        cRect(LeftDecor, TopDecor + BottomDecor, Size, Size), ColorBg, -1);
    // bottom,right corner
    decorPixmap->DrawEllipse(
        cRect(LeftDecor + Size + Width, TopDecor + BottomDecor, Size, Size),
        ColorBg, -2);
  ...
  case 4: // rect + alpha blend
    // top
    DecorDrawGlowRectHor(decorPixmap, LeftDecor + Size, TopDecor,
                         WidthDecor - Size * 2, Size, ColorBg);
    // bottom
    DecorDrawGlowRectHor(decorPixmap, LeftDecor + Size, TopDecor + BottomDecor,
                         WidthDecor - Size * 2, -1 * Size, ColorBg);
    // left
    DecorDrawGlowRectVer(decorPixmap, LeftDecor, TopDecor + Size, Size,
                         HeightDecor - Size * 2, ColorBg);
    // right
    DecorDrawGlowRectVer(decorPixmap, LeftDecor + Size + Width, TopDecor + Size,
                         -1 * Size, HeightDecor - Size * 2, ColorBg);

    DecorDrawGlowRectTL(decorPixmap, LeftDecor, TopDecor, Size, Size, ColorBg);
    DecorDrawGlowRectTR(decorPixmap, LeftDecor + Size + Width, TopDecor, Size,
                        Size, ColorBg);
    DecorDrawGlowRectBL(decorPixmap, LeftDecor, TopDecor + Size + Height, Size,
                        Size, ColorBg);
    DecorDrawGlowRectBR(decorPixmap, LeftDecor + Size + Width,
                        TopDecor + Size + Height, Size, Size, ColorBg);
    break;
  case 5: // round + alpha blend
    ...
    DecorDrawGlowEllipseTL(decorPixmap, LeftDecor, TopDecor, Size, Size,
                           ColorBg, 2);
    DecorDrawGlowEllipseTR(decorPixmap, LeftDecor + Size + Width, TopDecor,
                           Size, Size, ColorBg, 1);
    DecorDrawGlowEllipseBL(decorPixmap, LeftDecor, TopDecor + Size + Height,
                           Size, Size, ColorBg, 3);
    DecorDrawGlowEllipseBR(decorPixmap, LeftDecor + Size + Width,
                           TopDecor + Size + Height, Size, Size, ColorBg, 4);
    break;
  case 6: // invert round + alpha blend
    ...
    DecorDrawGlowEllipseBL(decorPixmap, LeftDecor, TopDecor + Size + Height,
                           Size, Size, ColorBg, -1);
    DecorDrawGlowEllipseBR(decorPixmap, LeftDecor + Size + Width,
                           TopDecor + Size + Height, Size, Size, ColorBg, -2);
    break;
  }
}
...
void cFlatBaseRender::DecorDrawGlowRectHor(cPixmap *pixmap, int Left, int Top,
...
void cFlatBaseRender::DecorDrawGlowRectVer(cPixmap *pixmap, int Left, int Top,
                                           int Width, int Height,
                                           tColor ColorBg) {
  double Alpha;
  if (Width < 0) {
...
void cFlatBaseRender::DecorDrawGlowRectTL(cPixmap *pixmap, int Left, int Top,
                                          int Width, int Height,
                                          tColor ColorBg) {
  double Alpha;
...
void cFlatBaseRender::DecorDrawGlowRectTR(cPixmap *pixmap, int Left, int Top,
                                          int Width, int Height,
                                          tColor ColorBg) {
  double Alpha;
...
void cFlatBaseRender::DecorDrawGlowRectBL(cPixmap *pixmap, int Left, int Top,
                                          int Width, int Height,
                                          tColor ColorBg) {
  double Alpha;
...
void cFlatBaseRender::DecorDrawGlowRectBR(cPixmap *pixmap, int Left, int Top,
                                          int Width, int Height,
                                          tColor ColorBg) {
  double Alpha;
...
void cFlatBaseRender::DecorDrawGlowEllipseTL(cPixmap *pixmap, int Left, int Top,
                                             int Width, int Height,
  ...
  for (int i = 0, j = Width; i < Width; i++, j--) {
    if (VDRVERSNUM < 20002 &&
        j == 1) // in VDR Version < 2.0.2 osd breaks if width & height == 1
      continue;
    Alpha = 255 / Width * i;
    tColor col = SetAlpha(ColorBg, 100.0 / 255.0 * Alpha / 100.0);
...
void cFlatBaseRender::DecorDrawGlowEllipseTR(cPixmap *pixmap, int Left, int Top,
                                             int Width, int Height,
                                             tColor ColorBg, int type) {
  double Alpha;
...
void cFlatBaseRender::DecorDrawGlowEllipseBL(cPixmap *pixmap, int Left, int Top,
                                             int Width, int Height,
                                             tColor ColorBg, int type) {
  double Alpha;
...
void cFlatBaseRender::DecorDrawGlowEllipseBR(cPixmap *pixmap, int Left, int Top,
                                             int Width, int Height,
                                             tColor ColorBg, int type) {
  double Alpha;
...
int cFlatBaseRender::GetFontAscender(const char *Name, int CharHeight,
                                     int CharWidth) {
  FT_Library library;
  FT_Face face;
  ...
  if (!error) {
    ...
    if (!error) {
      ...
      esyslog("ERROR: FreeType: load error %d (font = %s)", error,
              *fontFileName);
  } else
    esyslog("ERROR: FreeType: initialization error %d (font = %s)", error,
            *fontFileName);
...
void cFlatBaseRender::DrawWidgetWeather(void) {
  ...
  double p = 0.0;
  filename =
      cString::sprintf("%s/weather/weather.0.precipitation", WIDGETOUTPUTPATH);
  file.open(*filename, std::ifstream::in);
  if (file.is_open()) {
  ...
  filename =
      cString::sprintf("%s/weather/weather.1.precipitation", WIDGETOUTPUTPATH);
  file.open(*filename, std::ifstream::in);
  if (file.is_open()) {
  ...
  int widthTempTomorrow =
      std::max(weatherFontSml->Width(tempMaxTomorrow.c_str()),
               weatherFontSml->Width(tempMinTomorrow.c_str()));

  int wTop = topBarHeight + Config.decorBorderTopBarSize * 2 + 20 +
             Config.decorBorderChannelEPGSize;
  int wWidth = marginItem + weatherFont->Width(tempToday.c_str()) +
               weatherFontSign->Width(tempTodaySign.c_str()) + marginItem * 2 +
               weatherFont->Height() + marginItem + widthTempToday +
               marginItem + weatherFont->Height() - marginItem * 2 +
               weatherFontSml->Width(precToday.c_str()) + marginItem * 4 +
               weatherFont->Height() + marginItem + widthTempTomorrow +
               marginItem + weatherFont->Height() - marginItem * 2 +
  ...
  weatherWidget.AddText(tempToday.c_str(), false, cRect(left, 0, 0, 0),
                        Theme.Color(clrChannelFontEpg),
  ...
  int t = (weatherFont->Height() - fontAscender) -
          (weatherFontSign->Height() - fontAscender2);

  weatherWidget.AddText(tempTodaySign.c_str(), false, cRect(left, t, 0, 0),
                        Theme.Color(clrChannelFontEpg),
  ...
  cImage *img = imgLoader.LoadIcon(*weatherIcon, weatherFont->Height(),
                                   weatherFont->Height() - marginItem * 2);
  if (img) {
    weatherWidget.AddImage(img,
                           cRect(left, 0 + marginItem, weatherFont->Height(),
                                 weatherFont->Height()));
    left += weatherFont->Height() + marginItem;
  }
  weatherWidget.AddText(tempMaxToday.c_str(), false, cRect(left, 0, 0, 0),
                        Theme.Color(clrChannelFontEpg),
                        Theme.Color(clrItemCurrentBg), weatherFontSml,
                        widthTempToday, weatherFontSml->Height(), taRight);
  weatherWidget.AddText(tempMinToday.c_str(), false,
                        cRect(left, 0 + weatherFontSml->Height(), 0, 0),
                        Theme.Color(clrChannelFontEpg),
                        Theme.Color(clrItemCurrentBg), weatherFontSml,
                        widthTempToday, weatherFontSml->Height(), taRight);
  left += widthTempToday + marginItem;

  img = imgLoader.LoadIcon("widgets/umbrella", weatherFont->Height(),
                           weatherFont->Height() - marginItem * 2);
  if (img) {
    weatherWidget.AddImage(img,
                           cRect(left, 0 + marginItem, weatherFont->Height(),
  ...
  weatherWidget.AddText(
      precToday.c_str(), false,
      cRect(left,
            0 + (weatherFont->Height() / 2 - weatherFontSml->Height() / 2), 0,
            0),
      Theme.Color(clrChannelFontEpg), Theme.Color(clrItemCurrentBg),
      weatherFontSml);
  left += weatherFontSml->Width(precToday.c_str()) + marginItem * 4;

  weatherWidget.AddRect(cRect(left - marginItem * 2, 0,
                              wWidth - left + marginItem * 2,
                              weatherFont->Height()),
                        Theme.Color(clrChannelBg));

  weatherIcon = cString::sprintf("widgets/%s", iconTomorrow.c_str());
  img = imgLoader.LoadIcon(*weatherIcon, weatherFont->Height(),
                           weatherFont->Height() - marginItem * 2);
  if (img) {
    weatherWidget.AddImage(img,
                           cRect(left, 0 + marginItem, weatherFont->Height(),
  ...
  weatherWidget.AddText(tempMaxTomorrow.c_str(), false, cRect(left, 0, 0, 0),
                        Theme.Color(clrChannelFontEpg),
                        Theme.Color(clrChannelBg), weatherFontSml,
                        widthTempTomorrow, weatherFontSml->Height(), taRight);
  weatherWidget.AddText(tempMinTomorrow.c_str(), false,
                        cRect(left, 0 + weatherFontSml->Height(), 0, 0),
                        Theme.Color(clrChannelFontEpg),
                        Theme.Color(clrChannelBg), weatherFontSml,
                        widthTempTomorrow, weatherFontSml->Height(), taRight);
  ...
  weatherWidget.AddText(
      precTomorrow.c_str(), false,
      cRect(left,
            0 + (weatherFont->Height() / 2 - weatherFontSml->Height() / 2), 0,
            0),
      Theme.Color(clrChannelFontEpg), Theme.Color(clrChannelBg),
      weatherFontSml);
